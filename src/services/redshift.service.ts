...
export class RedshiftService {
   ...
   * 
   * @deprecated Use insertBatchData for better performance
   */
  ...
  /**
   * 🚀 BATCH INSERT - Insert multiple AppsFlyer events in a single query
   * This is 10-100x more efficient than individual inserts
   * 
   * @param dataArray - Array of AppsFlyer events to insert
   * @returns Statement ID from primary Redshift
   */
  async insertBatchData(dataArray: AppsFlyerEvent[]): Promise<string> {
    ...
    try {
      ...
      // Insert into both primary and secondary Redshift instances in parallel
      const insertPromises = [
        this.batchInsertToRedshift(parsedDataArray, this.redshiftClientFactory, 'primary')
      ];

      // Add secondary Redshift insert if enabled
      if (this.secondaryRedshiftClientFactory.isEnabled()) {
        insertPromises.push(
          this.batchInsertToRedshift(parsedDataArray, this.secondaryRedshiftClientFactory, 'secondary')
        );
      }
      ...
      results.forEach((result, index) => {
        const target = index === 0 ? 'primary' : 'secondary';
        if (result.status === 'fulfilled') {
          this.logger.info(`✅ Batch inserted ${parsedDataArray.length} rows into ${target} Redshift`, {
            statementId: result.value,
            rowCount: parsedDataArray.length,
            target,
          });
        } else {
          this.logger.error(`❌ Failed to batch insert into ${target} Redshift`, result.reason, {
            rowCount: parsedDataArray.length,
            target,
          });
        }
      ...
      this.logger.error('Error processing batch data for Redshift insertion', error, {
  ...
  /**
   * 🚀 BATCH INSERT - Insert multiple rows into Redshift in a single query
   * This dramatically reduces API calls and improves throughput
   */
  private async batchInsertToRedshift(
    parsedDataArray: AppsFlyerEvent[],
    clientFactory: RedshiftClientFactory | SecondaryRedshiftClientFactory,
    target: 'primary' | 'secondary'
  ): Promise<string> {
    const client = clientFactory.getClient();
    ...
    try {
      // Helper functions (same as single insert)
      const formatValue = (value: any): string => {
        if (value === null || value === undefined || value === '') {
          return 'NULL';
        }
      ...
      // Build VALUES rows for each data item
      const valuesRows = parsedDataArray.map(parsedData => {
        return `(
          ${formatValue(parsedData.appsflyer_id)}, ${formatValue(parsedData.event_name)}, 
          ${formatTimestamp(parsedData.event_time)}, ${formatTimestamp(parsedData.event_time_selected_timezone)},
          ${formatValue(parsedData.customer_user_id)}, ${formatValue(parsedData.idfa)}, ${formatValue(parsedData.idfv)}, 
      ...
      // Build complete BATCH INSERT statement
      const sql = `
        INSERT INTO ${config.tableName} (
          appsflyer_id, event_name, event_time, event_time_selected_timezone,
          customer_user_id, idfa, idfv, advertising_id, android_id, oaid, amazon_aid, imei,
      ...
      this.logger.debug(`🚀 Batch insert command sent to ${target} Redshift (${parsedDataArray.length} rows, async)`, {
        statementId,
        tableName: config.tableName,
        rowCount: parsedDataArray.length,
        target,
      ...
      this.logger.error(`❌ Error batch inserting into ${target} Redshift`, error, {
        tableName: config.tableName,
        rowCount: parsedDataArray.length,
        target,
      });
